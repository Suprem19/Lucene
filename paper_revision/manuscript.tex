...
\begin{table*}[htbp]
\centering
\caption{\hl{Five-Seed Startup, Safety and Effect-Size Results at the Anchor Condition (Scenario~1, 50\% CAV, 30\% Bernoulli Packet Loss)}}
...
\rowcolor{yellow}Spacing-RMSE difference vs.\ MHA (m) [95\% CI] & $8.28^{\dagger}$ [7.94, 8.62] & $6.61^{\dagger}$ [6.28, 6.94] & $3.76^{\dagger}$ [3.45, 4.07] & $4.61$ [3.58, 5.64] & $3.68$ [2.87, 4.49] & $0.57$ [0.08, 1.06] & $1.94$ [1.29, 2.59] & -- \\ \hline %reference
\rowcolor{yellow}Hedges' $g$ (seed level) & n/a & n/a & n/a & $6.6$ & $6.5$ & $1.5$ & $4.1$ & -- \\ \hline %reference
\end{tabular}%
}
...
{\raggedright
\footnotesize
\hl{\textit{Note:} Learning-based methods: mean$\pm$std over five independent training seeds, each averaged over 20 evaluation episodes with common random numbers; model-based controllers: mean$\pm$std over the same five groups of 20 realizations. $T_{90}$: time for the mean CAV speed to reach 22.5~m/s; $T_{\rm s}$: earliest time after which all CAVs stay within $\pm0.5$~m/s of 25~m/s for 2~s; n.r.: not reached within 60~s in any group. Spacing-RMSE differences are baseline minus MHA+Distribution, so positive values favour MHA+Distribution. For learning-based baselines, the interval is Welch's 95\% $t$-interval on seed-level means. $^{\dagger}$Model-based controllers: descriptive paired difference over the 100 matched realizations with a hierarchical bootstrap interval; no hypothesis test or standardized effect size is attached, because their spread does not include training variability.}
\par}
\end{table*}
