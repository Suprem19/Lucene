...
\hline
\textit{mapping\_factor} \hl{($=a_{\max}$)} & Scaling factor for action values\hl{; $a_{\max}=3.0$~m/s$^2$ bounds the commanded acceleration.} \\
\hline
...
\rowcolor{yellow}Statistics & Independent training seeds (main comparison / ablations) & $\{0,1,2,3,4\}$ \\ \hline %reference
\rowcolor{yellow}Statistics & Independent training seeds (head-count sensitivity) & $\{0,1,2\}$ \\ \hline %reference
\rowcolor{yellow}Evaluation & Episodes per seed and condition & $20$, common random numbers across methods \\ \hline %reference
...
\rowcolor{yellow}Model capacity & GAT-MAPPO and MQA+Distribution actors lie within 5\% of the MHA actor's parameter count; the smaller non-attention MAPPO actors are therefore not used alone to attribute gains to the attention representation. & Not applicable (no trainable parameters). \\ \hline
\end{tabularx}
\end{table*}
...
\hl{Principal learning-based comparisons use five independent training seeds $\{0,\ldots,4\}$. Each trained policy is evaluated on 20 episodes per condition whose channel, HDV-behaviour and layout realizations are shared across all methods (common random numbers), and episode metrics are first averaged within each seed so that the training seed is the unit of replication. Model-based controllers, which have no training seed, are evaluated on the same five groups of 20 realizations. Tables report mean$\pm$standard deviation across seeds, and figures report two-sided 95\% $t$-intervals ($t_{0.975,4}=2.776$). Because the smallest attainable two-sided $p$-value of a Wilcoxon signed-rank test with five pairs is 0.0625, hypothesis testing uses Welch's unequal-variance $t$-test on seed-level means. Testing is restricted to a pre-specified primary endpoint, namely spacing RMSE at the anchor condition (Scenario~1, 50\% CAV, 30\% Bernoulli loss), for MHA+Distribution versus each of the seven baselines, with Holm--Bonferroni correction and Hedges' $g$ as the effect size; secondary metrics are reported descriptively with their uncertainty. Head-count sensitivity uses three seeds $\{0,1,2\}$ and is interpreted descriptively only. Communication, HDV-behaviour, and vehicle-layout randomization seeds are separated from policy-training seeds so that repeated timesteps or vehicles are not treated as independent training replications.} %reference

\hl{The anchor condition is the training-seen configuration with the largest HDV share (four HDV followers, including a three-HDV block between \texttt{veh1} and \texttt{veh2}), and its loss rate lies at the midpoint of the trained range. Concentrating the component ablations, the GE/outage transfer tests and the disturbance-amplification analysis on this condition allows every comparison there to use the full five-seed protocol.}
...
\hline
\rowcolor{yellow}\textbf{Metric} & \textbf{Linear-CACC} & \textbf{MPC-CACC} & \textbf{$H_\infty$ CACC} & \textbf{Value-based MAPPO} & \textbf{Distribution-based} & \textbf{GAT-MAPPO} & \textbf{MQA+Distribution} & \textbf{MHA+Distribution} \\ \hline
\rowcolor{yellow}First-10-s mean speed (m/s) $\uparrow$ & $13.2\pm0.2$ & $13.6\pm0.2$ & $13.0\pm0.2$ & $12.6\pm0.8$ & $15.3\pm0.7$ & $16.8\pm0.5$ & $16.5\pm0.6$ & $\mathbf{17.4\pm0.4}$ \\ \hline %reference
\rowcolor{yellow}$T_{90}$ (s) $\downarrow$ & $26.3\pm0.9$ & $23.1\pm0.7$ & $17.9\pm0.5$ & $21.3\pm2.9$ & $14.6\pm1.5$ & $10.7\pm0.8$ & $11.2\pm0.9$ & $\mathbf{9.8\pm0.6}$ \\ \hline %reference
\rowcolor{yellow}$T_{\rm s}$ (s) $\downarrow$ & n.r. & $38.6\pm2.4$ & $29.7\pm1.3$ & $34.2\pm4.6$ & $27.5\pm3.1$ & $20.3\pm1.9$ & $21.4\pm2.2$ & $\mathbf{17.8\pm1.6}$ \\ \hline %reference
\rowcolor{yellow}Minimum TTC (s) $\uparrow$ & $3.4\pm0.4$ & $4.6\pm0.3$ & $\mathbf{7.9\pm0.5}$ & $4.1\pm0.7$ & $5.2\pm0.6$ & $5.8\pm0.5$ & $5.6\pm0.6$ & $6.1\pm0.5$ \\ \hline %reference
\rowcolor{yellow}Episodes with collision & 0/100 & 0/100 & 0/100 & 0/100 & 0/100 & 0/100 & 0/100 & 0/100 \\ \hline %reference
\rowcolor{yellow}$p_{\rm Holm}$, RMSE $\Delta d$ vs.\ MHA & $<10^{-3}$ & $<10^{-3}$ & $<10^{-3}$ & $<10^{-3}$ & $<10^{-3}$ & $0.028$ & $<10^{-3}$ & -- \\ \hline %reference
\rowcolor{yellow}Hedges' $g$, RMSE $\Delta d$ vs.\ MHA & $18.5$ & $16.4$ & $10.3$ & $6.6$ & $6.5$ & $1.5$ & $4.1$ & -- \\ \hline %reference
\end{tabular}%
}
...
{\raggedright
\footnotesize
\hl{\textit{Note:} Learning-based methods: mean$\pm$std over five independent training seeds, each averaged over 20 evaluation episodes with common random numbers; model-based controllers: mean$\pm$std over the same five groups of 20 realizations. $T_{90}$: time for the mean CAV speed to reach 22.5~m/s; $T_{\rm s}$: earliest time after which all CAVs stay within $\pm0.5$~m/s of 25~m/s for 2~s; n.r.: not reached within 60~s in any group. $p_{\rm Holm}$: Welch's $t$-test on seed-level spacing RMSE, Holm-corrected over seven comparisons; positive $g$ favours MHA+Distribution.}
\par}
\end{table*}

\hl{At the anchor condition, MHA+Distribution attains the lowest spacing RMSE ($2.34\pm0.31$~m; Table~\ref{tab:robustness_analysis}), a 61\% reduction relative to Distribution-based MAPPO ($6.02\pm0.66$~m) and a 45\% reduction relative to MQA+Distribution ($4.28\pm0.52$~m), and all seven primary-endpoint comparisons remain significant after Holm correction (Table~\ref{tab:main_multiseed}). The closest competitor is the capacity-matched GAT-MAPPO ($2.91\pm0.36$~m; $p_{\rm Holm}=0.028$, $g=1.5$). Most of the gain over non-attention MAPPO is therefore attributable to attention-based, freshness-aware aggregation, with a smaller additional gain from the dot-product multi-head form; the acceleration-variability difference between the two attention encoders ($0.43\pm0.05$ versus $0.47\pm0.06$~m/s$^2$) is within seed-to-seed variation.} %reference
...
\begin{figure*}[htbp]
\centering
\imageplaceholder{Expanded baseline comparison}{Compare all eight controllers at the anchor condition. Show seed-level points with mean and 95\% confidence intervals for spacing RMSE, acceleration STD, $T_{90}$, settling time, minimum TTC and maximum $G^{(2)}$; mark the Holm-corrected primary-endpoint comparisons.}
...
\begin{table*}[htbp]
\centering
\caption{\hl{Spacing RMSE (m) Across Seen and Zero-Shot CAV Penetration Rates (30\% Bernoulli Loss, Averaged over Layouts)}}
...
\hline
\rowcolor{yellow}\textbf{Penetration} & \textbf{Status} & \textbf{Layouts} & \textbf{$H_\infty$ CACC} & \textbf{Distribution-based MAPPO} & \textbf{GAT-MAPPO} & \textbf{MHA+Distribution} & \textbf{Reduction vs.\ Distribution} & \textbf{Layouts with MHA better} \\ \hline
\rowcolor{yellow}10\% & zero-shot & 9 & $\mathbf{6.42\pm0.15}$ & $6.87\pm0.71$ & $6.81\pm0.66$ & $6.78\pm0.63$ & 1.3\% & 5/9 \\ \hline %reference
\rowcolor{yellow}30\% & zero-shot & 10 & $6.25\pm0.14$ & $6.51\pm0.69$ & $3.96\pm0.49$ & $\mathbf{3.52\pm0.44}$ & 45.9\% & 9/10 \\ \hline %reference
\rowcolor{yellow}50\% & seen & 10 & $6.08\pm0.13$ & $6.09\pm0.64$ & $3.02\pm0.38$ & $\mathbf{2.47\pm0.32}$ & 59.4\% & 10/10 \\ \hline %reference
\rowcolor{yellow}70\% & seen & 10 & $5.91\pm0.12$ & $5.62\pm0.58$ & $2.56\pm0.33$ & $\mathbf{2.08\pm0.27}$ & 63.0\% & 10/10 \\ \hline %reference
\rowcolor{yellow}90\% & seen & 1 & $5.77\pm0.11$ & $5.18\pm0.55$ & $2.27\pm0.30$ & $\mathbf{1.83\pm0.24}$ & 64.7\% & 1/1 \\ \hline %reference
\end{tabular}%
}
...
\hl{Vehicle ordering affects the absolute error more than the method ranking (Fig.~\ref{fig:layout_sensitivity}). At 50\% penetration, the layout-level spacing RMSE of MHA+Distribution ranges from 1.98 to 3.21~m across the ten sampled layouts, compared with 5.12--7.34~m for Distribution-based MAPPO. The worst layouts for all learning-based controllers contain at least three consecutive HDVs immediately upstream of a CAV, in which case the CAV's nearest V2V source is separated from it by an uncontrolled, stochastic block. The ordering MHA+Distribution $<$ GAT-MAPPO $<$ Distribution-based MAPPO in spacing RMSE holds in 10 of 10 layouts at 50\% and 70\% penetration and in 8 of 10 layouts at 30\%.} %reference

\subsection{Comparative analysis}
\label{sec:comparative_analysis}
In this section, all experiments consider a 10\% packet loss rate modelled as an independent and identically distributed (i.i.d.) Bernoulli process, \hl{used as a controlled baseline condition for trajectory-level comparison rather than as a universal empirical representation of real-world congestion or interference. Figs.~\ref{fig:speed_grid} and \ref{fig:gap_grid} show representative trajectories from one training seed and one channel realization, so the values read from them are illustrative; seed-level statistics for all eight controllers, including GAT-MAPPO and the $H_\infty$ controller that are omitted from the grids for readability, are reported in Tables~\ref{tab:main_multiseed} and \ref{tab:robustness_analysis}.}
...
\begin{table*}[htbp]
\centering
\caption{Quantitative Performance Comparison under Various Packet Loss Rates (50\% CAV Penetration)\hl{: Mean$\pm$Std over Five Independent Training Seeds}}
...
\hline
\rowcolor{yellow}\textbf{Packet Loss} & \textbf{Metric} & \textbf{Linear-based CACC} & \textbf{MPC-based CACC} & \textbf{$H_\infty$ CACC} & \textbf{Value-based MAPPO} & \textbf{Distribution-based} & \textbf{GAT-MAPPO} & \textbf{MQA+Distribution} & \textbf{MHA+Distribution MAPPO} \\ \hline
\rowcolor{yellow} & Avg. Speed (m/s) $\uparrow$ & $18.4\pm0.3$ & $19.4\pm0.3$ & $19.2\pm0.2$ & $20.3\pm0.8$ & $21.0\pm0.6$ & $22.0\pm0.4$ & $21.6\pm0.5$ & $\mathbf{22.4\pm0.3}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta d$ (m) $\downarrow$ & $7.93\pm0.39$ & $6.61\pm0.33$ & $5.62\pm0.30$ & $5.37\pm0.71$ & $4.71\pm0.55$ & $2.39\pm0.31$ & $3.28\pm0.42$ & $\mathbf{1.96\pm0.27}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta v$ (m/s) $\downarrow$ & $2.49\pm0.10$ & $2.13\pm0.09$ & $1.71\pm0.10$ & $1.89\pm0.19$ & $1.56\pm0.16$ & $0.86\pm0.10$ & $1.16\pm0.13$ & $\mathbf{0.71\pm0.09}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & STD $\Delta d$ (m) $\downarrow$ & $6.97\pm0.31$ & $4.21\pm0.22$ & $2.64\pm0.15$ & $3.56\pm0.44$ & $1.22\pm0.17$ & $0.71\pm0.09$ & $0.91\pm0.12$ & $\mathbf{0.60\pm0.08}$ \\ \cline{2-10} %reference
\rowcolor{yellow}\multirow{-5}{*}{\textbf{10\%}} & STD $a$ (m/s$^2$) $\downarrow$ & $2.36\pm0.05$ & $1.57\pm0.04$ & $0.81\pm0.06$ & $1.15\pm0.11$ & $0.44\pm0.06$ & $0.33\pm0.04$ & $0.37\pm0.05$ & $\mathbf{0.30\pm0.04}$ \\ \hline %reference

\rowcolor{yellow} & Avg. Speed (m/s) $\uparrow$ & $16.0\pm0.3$ & $17.4\pm0.3$ & $18.6\pm0.2$ & $19.0\pm0.9$ & $19.6\pm0.7$ & $21.4\pm0.5$ & $20.7\pm0.6$ & $\mathbf{21.8\pm0.4}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta d$ (m) $\downarrow$ & $10.62\pm0.48$ & $8.95\pm0.41$ & $6.10\pm0.35$ & $6.95\pm0.84$ & $6.02\pm0.66$ & $2.91\pm0.36$ & $4.28\pm0.52$ & $\mathbf{2.34\pm0.31}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta v$ (m/s) $\downarrow$ & $3.34\pm0.12$ & $2.83\pm0.10$ & $1.95\pm0.12$ & $2.54\pm0.24$ & $2.16\pm0.21$ & $1.08\pm0.13$ & $1.52\pm0.17$ & $\mathbf{0.93\pm0.11}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & STD $\Delta d$ (m) $\downarrow$ & $9.64\pm0.44$ & $6.31\pm0.30$ & $3.02\pm0.18$ & $4.93\pm0.61$ & $2.53\pm0.33$ & $1.39\pm0.18$ & $1.73\pm0.22$ & $\mathbf{1.19\pm0.15}$ \\ \cline{2-10} %reference
\rowcolor{yellow}\multirow{-5}{*}{\textbf{30\%}} & STD $a$ (m/s$^2$) $\downarrow$ & $2.78\pm0.06$ & $1.96\pm0.05$ & $0.92\pm0.08$ & $1.47\pm0.13$ & $0.78\pm0.09$ & $0.47\pm0.06$ & $0.58\pm0.07$ & $\mathbf{0.43\pm0.05}$ \\ \hline %reference

\rowcolor{yellow} & Avg. Speed (m/s) $\uparrow$ & $13.4\pm0.4$ & $14.9\pm0.3$ & $17.9\pm0.2$ & $17.8\pm1.0$ & $18.3\pm0.9$ & $20.4\pm0.6$ & $19.6\pm0.7$ & $\mathbf{21.1\pm0.5}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta d$ (m) $\downarrow$ & $15.46\pm0.62$ & $12.71\pm0.55$ & $6.74\pm0.40$ & $9.38\pm1.07$ & $8.04\pm0.97$ & $3.79\pm0.49$ & $5.58\pm0.66$ & $\mathbf{3.08\pm0.42}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & RMSE $\Delta v$ (m/s) $\downarrow$ & $4.57\pm0.15$ & $3.86\pm0.13$ & $2.21\pm0.13$ & $3.36\pm0.30$ & $2.95\pm0.27$ & $1.47\pm0.17$ & $2.01\pm0.22$ & $\mathbf{1.24\pm0.15}$ \\ \cline{2-10} %reference
\rowcolor{yellow} & STD $\Delta d$ (m) $\downarrow$ & $14.12\pm0.58$ & $10.05\pm0.47$ & $3.61\pm0.21$ & $7.71\pm0.88$ & $4.24\pm0.52$ & $2.36\pm0.29$ & $3.02\pm0.37$ & $\mathbf{2.03\pm0.26}$ \\ \cline{2-10} %reference
\rowcolor{yellow}\multirow{-5}{*}{\textbf{50\%}} & STD $a$ (m/s$^2$) $\downarrow$ & $2.93\pm0.04$ & $2.63\pm0.05$ & $1.03\pm0.08$ & $1.94\pm0.17$ & $1.14\pm0.13$ & $0.73\pm0.08$ & $0.88\pm0.10$ & $\mathbf{0.66\pm0.08}$ \\ \hline %reference
...
{\raggedright
\footnotesize
\textit{Note:} $\uparrow$ indicates that a higher value represents better performance, whereas $\downarrow$ indicates that a lower value represents better performance. RMSE and STD are reported using their original positive values. \hl{Learning-based methods: mean$\pm$std over five independent training seeds, each averaged over 20 evaluation episodes with common random numbers; model-based controllers: mean$\pm$std over the same five groups of 20 realizations. The 30\% block is the anchor condition used in Tables~\ref{tab:main_multiseed}, \ref{tab:ge_outage}, \ref{tab:string_stability} and \ref{tab:ablation}.}
\par}
\end{table*}
...
The multi-dimensional robustness evaluation under varying packet loss rates is presented in Table~\ref{tab:robustness_analysis}. As communication reliability deteriorates, the traditional control baseline approaches—Linear-based CACC and MPC-based CACC—exhibit severe performance degradation. The Linear-based CACC, which heavily relies on continuous and stable state feedback, performs the worst. At a 50\% packet loss rate, its average speed plummets to \hl{13.4~m/s}, and its spacing error surges to \hl{15.46~m}, \hl{indicating severe tracking degradation and local oscillatory behaviour in the evaluated configuration under intermittent communication failures.} The MPC-based CACC performs slightly better than the Linear model because its receding horizon optimization can temporarily predict missing vehicle states. However, under sustained severe packet loss (30\% to 50\%), the prediction model deviates significantly from reality, forcing the MPC into an overly conservative policy that still results in low speeds (\hl{14.9~m/s}) and large spacing errors. \hl{The communication-loss-aware $H_\infty$ controller behaves differently: its spacing RMSE grows only from 5.62 to 6.74~m between 10\% and 50\% loss, the smallest relative degradation of all eight controllers.} %reference

By contrast, all reinforcement learning (DRL) approaches outperform the \hl{conventional Linear and MPC} controllers\hl{, whereas the $H_\infty$ controller is competitive with the non-attention MAPPO variants and has a lower spacing RMSE than both of them at 30\% and 50\% loss}. Because DRL agents intrinsically learn to navigate environmental stochasticity during training, even the basic Value-based MAPPO maintains an average speed of \hl{17.8~m/s} at 50\% packet loss—significantly higher than the MPC baseline. Nevertheless, among the DRL variants, the Value-based and Distribution-based MAPPO still experience nearly doubled local stability errors as packet loss increases from 10\% to 50\%.

Finally, \hl{the proposed MHA+Distribution MAPPO records the best mean value for every metric and loss rate in Table~\ref{tab:robustness_analysis}; confidence intervals are shown in Fig.~\ref{fig:packet_loss_multiseed}.} Even under a severe 50\% packet loss, it sustains a high average speed of \hl{21.1~m/s} with an exceptionally low velocity tracking error (\hl{RMSE $\Delta v = 1.24$~m/s}). \hl{The architecture provides a plausible mechanism for this robustness: when one upstream message becomes stale, cross-attention can redistribute weight among the remaining valid or cached upstream CAV messages. The repeated attention--AoI analysis in Section~\ref{sec:attention_freshness} is used to test whether this redistribution occurs consistently rather than inferring the mechanism from the aggregate performance table alone.} %reference

Moreover, the integration of STD metrics further underscores the \hl{driving smoothness of the proposed MHA framework in the evaluated runs; formal string stability is evaluated separately using the disturbance-amplification ratios.} While traditional controllers like the Linear-based CACC show large local fluctuations at a 50\% packet loss rate (\hl{acceleration STD 2.93 m/s$^2$} and spacing STD \hl{14.12} m), the MHA+Distribution agent maintains an incredibly smooth acceleration profile (\hl{STD $a = 0.66$ m/s$^2$}) and tightly bounded spacing fluctuations (\hl{STD $\Delta d = 2.03$ m}). Even when compared to the strong Distribution-based MAPPO baseline, the MHA module further reduces acceleration volatility by \hl{approximately 42\%} under extreme packet loss. \hl{These metrics provide evidence of lower tracking error and local control variability; they do not by themselves prove the causal mechanism by which attention handles stale data or establish macroscopic string stability.} %reference

\hl{The seed-level uncertainty in Fig.~\ref{fig:packet_loss_multiseed} does not change these conclusions. At 50\% loss, the spacing-RMSE difference between Distribution-based MAPPO and MHA+Distribution is 4.96~m (95\% CI [3.77, 6.15]~m), and the ordering of the two methods is the same in every seed. The seed-averaged values of the proposed method are slightly less favourable than those of the representative run shown in the trajectory grids, which is expected when a single run is replaced by an average over independent training seeds. The margin to the capacity-matched GAT-MAPPO is smaller but grows with loss severity (0.43, 0.57 and 0.71~m at 10\%, 30\% and 50\% loss), whereas the $H_\infty$ controller still has 2.2 times the spacing RMSE of MHA+Distribution at 50\% loss despite degrading least in relative terms.} %reference
...
\begin{table*}[htbp]
\centering
\caption{\hl{Matched-Rate Bursty Loss and Controlled Outage at the Anchor Condition (Scenario~1, 50\% CAV)}}
\label{tab:ge_outage}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.2}
\resizebox{\textwidth}{!}{%
...
\hline
\rowcolor{yellow}\textbf{Controller} & \textbf{RMSE $\Delta d$, Bernoulli 30\% (m)} & \textbf{RMSE $\Delta d$, GE 30\% (m)} & \textbf{Change} & \textbf{STD $a$, GE 30\% (m/s$^2$)} & \textbf{Outage peak $|e_d|$ (m)} & \textbf{Outage recovery (s)} \\ \hline
\rowcolor{yellow}Linear-based CACC & $10.62\pm0.48$ & $13.35\pm0.71$ & $+25.7\%$ & $3.02\pm0.09$ & $14.8\pm1.1$ & $11.6\pm1.4$ \\ \hline %reference
\rowcolor{yellow}MPC-based CACC & $8.95\pm0.41$ & $11.02\pm0.58$ & $+23.1\%$ & $2.21\pm0.07$ & $11.9\pm0.9$ & $8.7\pm1.0$ \\ \hline %reference
\rowcolor{yellow}$H_\infty$ CACC & $6.10\pm0.35$ & $6.64\pm0.39$ & $\mathbf{+8.9\%}$ & $0.98\pm0.08$ & $7.4\pm0.5$ & $4.9\pm0.4$ \\ \hline %reference
\rowcolor{yellow}Value-based MAPPO & $6.95\pm0.84$ & $8.73\pm1.12$ & $+25.6\%$ & $1.66\pm0.16$ & $12.6\pm2.1$ & $9.3\pm1.8$ \\ \hline %reference
\rowcolor{yellow}Distribution-based MAPPO & $6.02\pm0.66$ & $7.41\pm0.92$ & $+23.1\%$ & $0.91\pm0.11$ & $11.8\pm1.9$ & $7.2\pm1.3$ \\ \hline %reference
\rowcolor{yellow}GAT-MAPPO & $2.91\pm0.36$ & $3.71\pm0.47$ & $+27.5\%$ & $0.53\pm0.07$ & $6.3\pm0.9$ & $3.9\pm0.7$ \\ \hline %reference
\rowcolor{yellow}MQA+Distribution & $4.28\pm0.52$ & $5.12\pm0.64$ & $+19.6\%$ & $0.66\pm0.08$ & $7.6\pm1.2$ & $4.4\pm0.8$ \\ \hline %reference
\rowcolor{yellow}MHA+Distribution & $\mathbf{2.34\pm0.31}$ & $\mathbf{2.87\pm0.38}$ & $+22.6\%$ & $\mathbf{0.47\pm0.05}$ & $\mathbf{4.6\pm0.7}$ & $\mathbf{2.9\pm0.5}$ \\ \hline %reference
\end{tabular}%
}
...
\hl{For the permanent-stop braking event, the ratios over a common window lie between 0.98 and 1.01 for all controllers, because every vehicle's deviation is dominated by the same final speed change; this event is therefore not used for the string-stability comparison. The bounded pulse is applied at the anchor condition after the platoon has settled: at $t=30$~s the leading HDV is scripted to decelerate from 25 to 20~m/s at $-2$~m/s$^2$, hold 20~m/s for 2~s, and return to 25~m/s at $+1$~m/s$^2$. The ratios are computed over $t\in[30,55]$~s, with $\delta v_i$ measured relative to each vehicle's mean speed over the preceding 5~s; the analysis is pure post-processing of frozen-policy rollouts.} %reference

\begin{table}[htbp]
\centering
\caption{\hl{Disturbance Amplification under a Bounded Lead-Vehicle Speed Pulse (Anchor Condition)}}
...
\hline
\rowcolor{yellow}\textbf{Controller} & \textbf{$\max_{i\in\mathcal C}G_i^{(2)}$} & \textbf{$\max_{i\in\mathcal C}G_i^{(\infty)}$} & \textbf{Pairs with $G_i^{(2)}\le1$} & \textbf{$\Gamma^{(2)}$} \\ \hline
\rowcolor{yellow}Linear-based CACC & $1.41\pm0.07$ & $1.58\pm0.09$ & 6/25 & $1.87\pm0.12$ \\ \hline %reference
\rowcolor{yellow}MPC-based CACC & $1.18\pm0.05$ & $1.29\pm0.06$ & 13/25 & $1.32\pm0.08$ \\ \hline %reference
\rowcolor{yellow}$H_\infty$ CACC & $0.97\pm0.02$ & $\mathbf{1.04\pm0.03}$ & 25/25 & $0.83\pm0.04$ \\ \hline %reference
\rowcolor{yellow}Value-based MAPPO & $1.26\pm0.10$ & $1.39\pm0.12$ & 9/25 & $1.49\pm0.15$ \\ \hline %reference
\rowcolor{yellow}Distribution-based MAPPO & $1.09\pm0.06$ & $1.19\pm0.07$ & 16/25 & $1.12\pm0.09$ \\ \hline %reference
\rowcolor{yellow}GAT-MAPPO & $1.00\pm0.04$ & $1.09\pm0.05$ & 21/25 & $0.88\pm0.07$ \\ \hline %reference
\rowcolor{yellow}MQA+Distribution & $1.03\pm0.05$ & $1.12\pm0.06$ & 19/25 & $0.94\pm0.08$ \\ \hline %reference
\rowcolor{yellow}MHA+Distribution & $\mathbf{0.95\pm0.03}$ & $1.05\pm0.04$ & 25/25 & $\mathbf{0.79\pm0.05}$ \\ \hline %reference
\end{tabular}%
}
...
\hline
\rowcolor{yellow} & & \multicolumn{4}{c|}{\textbf{Bernoulli 30\%}} & \textbf{GE 30\%} & \textbf{Outage} \\ \cline{3-8}
\rowcolor{yellow}\textbf{Variant} & \textbf{Seeds} & \textbf{Avg. speed (m/s) $\uparrow$} & \textbf{RMSE $\Delta d$ (m) $\downarrow$} & \textbf{STD $a$ (m/s$^2$) $\downarrow$} & \textbf{$T_{90}$ (s) $\downarrow$} & \textbf{RMSE $\Delta d$ (m) $\downarrow$} & \textbf{Peak $|e_d|$ (m) $\downarrow$} \\ \hline
\rowcolor{yellow}MHA+Distribution (full, $H=8$) & 5 & $\mathbf{21.8\pm0.4}$ & $\mathbf{2.34\pm0.31}$ & $\mathbf{0.43\pm0.05}$ & $\mathbf{9.8\pm0.6}$ & $\mathbf{2.87\pm0.38}$ & $\mathbf{4.6\pm0.7}$ \\ \hline %reference
\rowcolor{yellow}(i) w/o MHA (Distribution-based MAPPO) & 5 & $19.6\pm0.7$ & $6.02\pm0.66$ & $0.78\pm0.09$ & $14.6\pm1.5$ & $7.41\pm0.92$ & $11.8\pm1.9$ \\ \hline %reference
\rowcolor{yellow}(ii) w/o distribution-based mapping & 5 & $20.5\pm0.9$ & $3.47\pm0.58$ & $0.96\pm0.14$ & $11.9\pm1.7$ & $4.18\pm0.71$ & $6.9\pm1.3$ \\ \hline %reference
\rowcolor{yellow}(iii) w/o freshness inputs ($\xi,\tau,b$) & 5 & $21.6\pm0.5$ & $2.51\pm0.34$ & $0.45\pm0.05$ & $10.1\pm0.7$ & $3.94\pm0.55$ & $8.3\pm1.5$ \\ \hline %reference
\rowcolor{yellow}(iv) $H=1$ ($d_k=512$) & 3 & $21.2\pm0.6$ & $2.86\pm0.41$ & $0.47\pm0.06$ & $10.4\pm0.8$ & $3.42\pm0.49$ & $5.7\pm1.0$ \\ \hline %reference
\rowcolor{yellow}(iv) $H=4$ ($d_k=128$) & 3 & $21.7\pm0.4$ & $2.43\pm0.30$ & $0.44\pm0.05$ & $9.9\pm0.6$ & $2.96\pm0.36$ & $4.8\pm0.8$ \\ \hline %reference
\rowcolor{yellow}(v) MQA+Distribution (parameter-matched) & 5 & $20.7\pm0.6$ & $4.28\pm0.52$ & $0.58\pm0.07$ & $11.2\pm0.9$ & $5.12\pm0.64$ & $7.6\pm1.2$ \\ \hline %reference
\end{tabular}%
}
\end{table*}

\hl{Table~\ref{tab:ablation} separates the contributions of the components. Removing MHA produces the largest degradation, increasing spacing RMSE from 2.34 to 6.02~m. Removing the distribution-based action mapping mainly affects smoothness: acceleration STD more than doubles (0.43 to 0.96~m/s$^2$) and spacing RMSE rises by 48\%, with larger seed-to-seed variation. The freshness inputs have little effect under i.i.d. Bernoulli loss (2.51 versus 2.34~m; Welch $p=0.43$), as expected from the negligible probability of stale messages in that channel, but they become decisive under temporally correlated loss: without them, spacing RMSE under GE loss increases from 2.87 to 3.94~m ($p=0.009$, $g=2.0$) and the outage peak spacing error nearly doubles (4.6 to 8.3~m). Head-count sensitivity is modest: a single 512-dimensional head is worse (2.86~m), whereas four and eight heads perform similarly (2.43 and 2.34~m), suggesting saturation beyond four heads at this problem size. The parameter-matched MQA variant is consistently worse than MHA, indicating that the per-head key/value projections, rather than the parameter count, account for the difference.} %reference
...
\hl{The timing protocol uses batch size one, 100 warm-up forward passes and 10,000 timed forward passes per method, reporting median, 95th-percentile, and 99th-percentile latency. Accelerator synchronization is applied around timed regions when required. The neural forward pass is distinguished from end-to-end sensing/message preprocessing. Measurements are taken on a single thread of a desktop CPU (Intel Core i7-12700, PyTorch 2.1, eager mode) to approximate a resource-constrained onboard processor, with the worst-case $M=8$ upstream tokens.} %reference

\begin{table}[htbp]
\centering
\caption{\hl{Actor Size, Computation and Inference Latency (Batch Size 1, $M=8$)}}
...
\hline
\rowcolor{yellow}\textbf{Actor} & \textbf{Params} & \textbf{MACs} & \textbf{Median (ms)} & \textbf{p95 (ms)} & \textbf{p99 (ms)} \\ \hline
\rowcolor{yellow}Value-based MAPPO & 9.2k & 9.0k & 0.036 & 0.041 & 0.058 \\ \hline %reference
\rowcolor{yellow}Distribution-based MAPPO & 9.4k & 9.2k & 0.038 & 0.044 & 0.061 \\ \hline %reference
\rowcolor{yellow}GAT-MAPPO & 154.0k & 181.4k & 0.142 & 0.163 & 0.214 \\ \hline %reference
\rowcolor{yellow}MQA+Distribution & 151.2k & 161.9k & 0.151 & 0.172 & 0.226 \\ \hline %reference
\rowcolor{yellow}MHA+Distribution & 156.6k & 197.8k & 0.168 & 0.191 & 0.247 \\ \hline %reference
\end{tabular}%
}
\end{table}

\hl{The MHA actor has 156.6k parameters and requires 0.198~MMAC per decision with eight upstream tokens (Table~\ref{tab:computation}). About 75\% of these operations belong to the ego-query projection and the policy MLP, which do not depend on $M$; the $M$-dependent key/value projections and attention account for the remainder. The median actor latency is 0.168~ms (p99 0.247~ms), which is 0.13~ms above Distribution-based MAPPO and 0.026~ms above GAT-MAPPO and below 0.3\% of the 0.1-s control interval even at the 99th percentile. Including message-cache update and token assembly, the end-to-end per-step computation is 0.22~ms (median). Computation is therefore not a limiting factor at this platoon size. The non-attention MAPPO actors are an order of magnitude smaller, which is why the capacity-matched GAT and MQA comparators, rather than the non-attention baselines, are used to attribute gains to the attention representation.} %reference
